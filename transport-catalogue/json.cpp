...
namespace json
{
	...
	namespace
	{
		...
		Node LoadNumber(std::istream& input)
		{
			...
			string parsed_numbers;

			// Считывает в parsed_numbers очередной символ из input
			auto read_char = [&parsed_numbers, &input]
			{
				parsed_numbers += static_cast<char>(input.get());
				if (!input)
				{
			...
			// Считывает одну или более цифр в parsed_numbers из input
...
// Пробуем преобразовать строку в int ,при неудаче пробуем преобразовать строку в double
			...
			try
			{
				...
				{
					number = stod(parsed_numbers);
				}

			}
			catch (...)
			{
				throw ParsingError("Failed to convert "s + parsed_numbers + " to number"s);
			}
