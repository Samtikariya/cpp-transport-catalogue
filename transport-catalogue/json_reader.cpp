...
{
	Queries queries = ParseBaseRequests(requests_.at("base_requests"));
...
void Reader::GetResponses(ostream& output)
{
	...
	MapRenderer mrenderer(render_settings);
	RequestHandler rhandler(tc_, mrenderer);
//	if (!requests_.at("stat_requests"s).AsArray().empty()){

//		}
	ExecuteStatRequests(requests_.at("stat_requests"s), rhandler, output);
}
